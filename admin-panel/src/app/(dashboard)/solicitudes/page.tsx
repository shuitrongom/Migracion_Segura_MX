    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
        ...
        <div className="relative overflow-hidden dark-card-static p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 group">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-medium text-white/70">Pagadas</p>
            ...
            <p className="text-2xl font-bold text-white">${solicitudes.filter(s => s.estatus === 'pagada').reduce((sum, s) => sum + Number(s.costo), 0).toLocaleString()}</p>
          </div>
        </div>
      </div>
      ...
      <div className="dark-card-static overflow-hidden hover:shadow-md transition-shadow duration-300">
        ...
        {loading ? (
          <div className="p-12 text-center text-white/70">Cargando...</div>
        ) : filtered.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-white/70 mt-1">Las solicitudes de los extranjeros aparecerán aquí</p>
          </div>
        ) : (
                ...
                <div key={sol.id} className="flex items-center justify-between px-6 py-4 hover:bg-[#222222] transition-colors group cursor-pointer" onClick={() => { setSelectedSolicitud(sol); setShowModal(true); }}>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-white capitalize">{getNombre(sol)}</p>
      ...
      {showModal && selectedSolicitud && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="bg-[#171717] rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[85vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b sticky top-0 bg-[#171717] z-10">
              <div>
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3 bg-[#1a1a1a] rounded-xl p-4">
                  {Object.entries(selectedSolicitud.datosFormulario || {}).map(([key, val]) => (
                    <div key={key}>
                      ...
                      <p className="text-sm text-white capitalize">{String(val) || '—'}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-r from-white/[0.02] to-white/[0.04] border">
                ...
                {selectedSolicitud.numeroPieza && <span className="text-sm font-mono text-amber-500">Pieza: {selectedSolicitud.numeroPieza}</span>}
                <span className="text-sm text-white/70 ml-auto">${selectedSolicitud.costo} MXN</span>
              </div>

              {/* Acciones según estatus */}
              {selectedSolicitud.estatus === 'pendiente_revision' && (
                <div className="space-y-4 border-t pt-4">
                  <h3 className="text-sm font-bold text-white flex items-center gap-2"><Send className="h-4 w-4 text-amber-500" /> Procesar solicitud</h3>
                  ...
                  <div>
                    ...
                    <input type="file" accept=".pdf" onChange={e => setPdfFile(e.target.files?.[0] || null)} className="w-full text-sm text-white/70 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-amber-500/10 file:text-amber-400 hover:file:bg-amber-500/10" />
                  </div>
              ...
              {selectedSolicitud.estatus === 'pendiente_pago' && (
                <div className="space-y-3 border-t pt-4">
                  <p className="text-sm text-white/70">Esperando pago del extranjero...</p>
