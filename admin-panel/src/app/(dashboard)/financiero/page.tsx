...
const ESTATUS_CONFIG: Record<string, { label: string; color: string; icon: typeof CheckCircle }> = {
  ...
  cancelado: { label: 'Cancelado', color: 'bg-white/[0.02] text-white/60 border-white/[0.08]', icon: XCircle },
};
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-[#171717] rounded-2xl border shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          ...
          <button onClick={() => pagosQuery.refetch()} className="p-2.5 rounded-xl hover:bg-white/[0.04] text-white/30 transition-colors">
              ...
              return (
                <div key={pago.id} className="flex items-center justify-between px-6 py-4 hover:bg-gradient-to-r hover:from-gray-50 hover:to-white transition-colors group">
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    <span className="text-xs font-medium text-amber-500 bg-amber-500/10 px-2.5 py-1 rounded-full border border-brand-100">
