...
const ESTATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  cerrado: { label: 'Cerrado', color: 'bg-[#1a1a1a] text-white/70 border-[#3a3a3a]' },
};
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="relative overflow-hidden dark-card-static p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 group">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-medium text-white/70">Abiertos</p>
        ...
        <div className="relative overflow-hidden dark-card-static p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 group">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-medium text-white/70">Resueltos</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 dark-card-static overflow-hidden hover:shadow-md transition-shadow duration-300">
          <div className="flex items-center gap-2 px-5 py-4 border-b">
            ...
            <span className="ml-auto text-xs font-medium text-white/70">{tickets.length}</span>
          </div>
          {ticketsQuery.isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-white/70 font-medium">Sin tickets</p>
            </div>
          ) : (
            <div className="divide-y divide-[#262626] max-h-[600px] overflow-y-auto">
              {tickets.map((ticket: any) => {
                ...
                return (
                  <button key={ticket.id} onClick={() => setSelectedTicket(ticket)} className={`w-full text-left px-5 py-4 hover:bg-[#222222] transition-colors ${selectedTicket?.id === ticket.id ? 'bg-amber-500/10/50 border-l-4 border-l-brand-500' : ''}`}>
                    ...
                    <p className="text-xs text-white/70 mt-1 line-clamp-1">{ticket.descripcion}</p>
                    <p className="text-[10px] text-white/70 mt-1.5">{ticket.createdAt?.slice(0, 10)}</p>
                  </button>
                );
              })}
            </div>
        ...
        <div className="lg:col-span-2 dark-card-static overflow-hidden hover:shadow-md transition-shadow duration-300">
          {!selectedTicket ? (
            <div className="p-16 text-center">
              ...
              <p className="text-white/70 font-medium">Selecciona un ticket</p>
              <p className="text-sm text-white/70 mt-1">Elige un ticket de la lista para ver los detalles</p>
            </div>
          ) : (
            <div className="flex flex-col h-[600px]">
              <div className="px-6 py-4 border-b flex items-center justify-between bg-gradient-to-r from-white/[0.02] to-white/[0.04]">
                <div>
                  ...
                  <p className="text-xs text-white/70 mt-0.5">{selectedTicket.createdAt?.slice(0, 10)}</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3">
                <div className="p-4 rounded-xl bg-gradient-to-r from-white/[0.02] to-white/[0.04] border">
                  ...
                  <p className="text-[10px] text-white/70 mt-2">Mensaje inicial del cliente</p>
                </div>
                {selectedTicket.mensajes?.map((msg: any) => (
                  <div key={msg.id} className={`p-4 rounded-xl ${msg.autorId === selectedTicket.clienteId ? 'bg-[#1a1a1a] border' : 'bg-gradient-to-r from-amber-500/10 to-amber-500/10 border border-amber-500/20 ml-8'}`}>
                    <p className="text-sm text-white/70">{msg.contenido}</p>
                    <p className="text-[10px] text-white/70 mt-2">{msg.createdAt?.slice(0, 16).replace('T', ' ')}</p>
                  </div>
                ))}
              </div>
              {selectedTicket.estatus !== 'cerrado' && (
                <div className="p-4 border-t bg-gradient-to-r from-white/[0.02] to-white/[0.04]">
                  <div className="flex gap-2">
                    <input type="text" value={respuesta} onChange={e => setRespuesta(e.target.value)} placeholder="Escribe tu respuesta..." className="flex-1 px-4 py-2.5 border border-[#3a3a3a] rounded-xl text-sm bg-[#171717] focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all" onKeyDown={e => { if (e.key === 'Enter' && respuesta.trim()) enviarRespuesta.mutate(); }} />
