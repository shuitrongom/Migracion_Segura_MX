...
const ESTATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  cerrado: { label: 'Cerrado', color: 'bg-gray-50 text-gray-600 border-gray-200' },
};
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="relative overflow-hidden bg-white rounded-2xl p-6 shadow-sm border hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-3xl font-bold text-gray-900">{abiertos}</p>
          </div>
        </div>
        <div className="relative overflow-hidden bg-white rounded-2xl p-6 shadow-sm border hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-3xl font-bold text-gray-900">{resueltos}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Lista de tickets */}
        <div className="lg:col-span-1 bg-white rounded-2xl border shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300">
          <div className="flex items-center gap-2 px-5 py-4 border-b">
            <div className="p-2 rounded-lg bg-brand-50"><Inbox className="h-4 w-4 text-brand-600" /></div>
            <h2 className="text-lg font-bold text-gray-900">Tickets</h2>
            <span className="ml-auto text-xs font-medium text-gray-400">{tickets.length}</span>
          </div>
          {ticketsQuery.isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-gray-500 font-medium">Sin tickets</p>
            </div>
          ) : (
            <div className="divide-y max-h-[600px] overflow-y-auto">
              {tickets.map((ticket: any) => {
                ...
                return (
                  <button key={ticket.id} onClick={() => setSelectedTicket(ticket)} className={`w-full text-left px-5 py-4 hover:bg-gradient-to-r hover:from-gray-50 hover:to-white transition-colors ${selectedTicket?.id === ticket.id ? 'bg-brand-50/50 border-l-4 border-l-brand-500' : ''}`}>
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-sm font-semibold text-gray-900 line-clamp-1">{ticket.asunto}</p>
                    ...
                    <p className="text-[10px] text-gray-400 mt-1.5">{ticket.createdAt?.slice(0, 10)}</p>
                  </button>
                );
              })}
            </div>
        ...
        {/* Detalle del ticket */}
        <div className="lg:col-span-2 bg-white rounded-2xl border shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300">
          {!selectedTicket ? (
            <div className="p-16 text-center">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center mx-auto mb-4">
                <MessageSquare className="h-8 w-8 text-gray-300" />
              </div>
              <p className="text-gray-500 font-medium">Selecciona un ticket</p>
              <p className="text-sm text-gray-400 mt-1">Elige un ticket de la lista para ver los detalles</p>
            </div>
          ) : (
            <div className="flex flex-col h-[600px]">
              <div className="px-6 py-4 border-b flex items-center justify-between bg-gradient-to-r from-gray-50 to-white">
                <div>
                  <p className="font-bold text-gray-900">{selectedTicket.asunto}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{selectedTicket.createdAt?.slice(0, 10)}</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3">
                <div className="p-4 rounded-xl bg-gradient-to-r from-gray-50 to-white border">
                  ...
                  <p className="text-[10px] text-gray-400 mt-2">Mensaje inicial del cliente</p>
                </div>
                {selectedTicket.mensajes?.map((msg: any) => (
                  <div key={msg.id} className={`p-4 rounded-xl ${msg.autorId === selectedTicket.clienteId ? 'bg-gray-50 border' : 'bg-gradient-to-r from-brand-50 to-amber-50 border border-brand-100 ml-8'}`}>
                    <p className="text-sm text-gray-700">{msg.contenido}</p>
                    <p className="text-[10px] text-gray-400 mt-2">{msg.createdAt?.slice(0, 16).replace('T', ' ')}</p>
                  </div>
                ))}
              </div>
              {selectedTicket.estatus !== 'cerrado' && (
                <div className="p-4 border-t bg-gradient-to-r from-gray-50 to-white">
                  <div className="flex gap-2">
                    <input type="text" value={respuesta} onChange={e => setRespuesta(e.target.value)} placeholder="Escribe tu respuesta..." className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all" onKeyDown={e => { if (e.key === 'Enter' && respuesta.trim()) enviarRespuesta.mutate(); }} />
                    <button onClick={() => { if (respuesta.trim()) enviarRespuesta.mutate(); }} disabled={!respuesta.trim()} className="px-5 py-2.5 bg-gradient-to-r from-brand-500 to-brand-600 text-white rounded-xl text-sm font-semibold hover:from-brand-600 hover:to-brand-700 disabled:opacity-50 shadow-lg shadow-brand-200/30 transition-all">
