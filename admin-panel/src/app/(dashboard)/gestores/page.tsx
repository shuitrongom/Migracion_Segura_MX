          ...
          <div className="p-6 space-y-4">
            {[1,2,3].map(i => (
              <div key={i} className="flex items-center gap-4 p-3 rounded-xl bg-white/[0.02]">
                <div className="h-11 w-11 rounded-full bg-white/[0.06] animate-pulse" />
              ...
              return (
                <div key={Gestor.id} className="flex items-center justify-between px-6 py-4 hover:bg-gradient-to-r hover:from-gray-50 hover:to-white transition-colors group">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="hidden sm:inline-flex px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-amber-500/10 text-amber-400 border border-brand-100">Gestor</span>
                    {isAdmin && (
                      <button
                        onClick={() => handleDelete(Gestor.id, Gestor.fullName)}
                        className="p-2 text-white/30 hover:text-red-500 rounded-lg hover:bg-red-50 transition-colors"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-[#171717] rounded-2xl shadow-2xl w-full max-w-lg mx-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              ...
              <button onClick={handleCloseModal} className="p-2 rounded-lg hover:bg-white/[0.04] text-white/30 transition-colors" aria-label="Cerrar">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <input id="fullName" type="text" value={formData.fullName} onChange={(e) => setFormData((prev) => ({ ...prev, fullName: e.target.value }))} className="w-full px-4 py-2.5 border border-white/[0.08] rounded-xl text-sm capitalize focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50 transition-all" placeholder="Nombre y apellidos" />
                </div>
                <div>
                  <label htmlFor="email" className="block text-xs font-medium text-white/60 mb-1.5">Correo electrónico *</label>
                  <input id="email" type="email" autoComplete="new-email" value={formData.email} onChange={(e) => setFormData((prev) => ({ ...prev, email: e.target.value }))} className="w-full px-4 py-2.5 border border-white/[0.08] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50 transition-all" placeholder="correo@ejemplo.com" />
                </div>
                <div>
                  <label htmlFor="phone" className="block text-xs font-medium text-white/60 mb-1.5">Teléfono *</label>
                  <input id="phone" type="tel" value={formData.phone} onChange={(e) => setFormData((prev) => ({ ...prev, phone: e.target.value }))} className="w-full px-4 py-2.5 border border-white/[0.08] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50 transition-all" placeholder="+52 55 1234 5678" />
                </div>
                <div>
                  <label htmlFor="sexo" className="block text-xs font-medium text-white/60 mb-1.5">Sexo</label>
                  <select id="sexo" value={formData.sexo} onChange={(e) => setFormData((prev) => ({ ...prev, sexo: e.target.value }))} className="w-full px-4 py-2.5 border border-white/[0.08] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50 transition-all">
                ...
                <div>
                  ...
                  <input id="curp" type="text" value={formData.curp} onChange={(e) => setFormData((prev) => ({ ...prev, curp: e.target.value.toUpperCase() }))} className="w-full px-4 py-2.5 border border-white/[0.08] rounded-xl text-sm uppercase focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50 transition-all" placeholder="18 caracteres" maxLength={18} />
                </div>
                <div>
                  <label htmlFor="rfc" className="block text-xs font-medium text-white/60 mb-1.5">RFC</label>
                  <input id="rfc" type="text" value={formData.rfc} onChange={(e) => setFormData((prev) => ({ ...prev, rfc: e.target.value.toUpperCase() }))} className="w-full px-4 py-2.5 border border-white/[0.08] rounded-xl text-sm uppercase focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50 transition-all" placeholder="13 caracteres" maxLength={13} />
                </div>
                <div className="md:col-span-2">
                  <label htmlFor="direccion" className="block text-xs font-medium text-white/60 mb-1.5">Dirección</label>
                  <input id="direccion" type="text" value={formData.direccion} onChange={(e) => setFormData((prev) => ({ ...prev, direccion: e.target.value }))} className="w-full px-4 py-2.5 border border-white/[0.08] rounded-xl text-sm capitalize focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50 transition-all" placeholder="Calle, número, colonia, ciudad" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input id="password" type={showPassword ? 'text' : 'password'} value={formData.password} onChange={(e) => setFormData((prev) => ({ ...prev, password: e.target.value }))} className="w-full px-4 py-2.5 pr-12 border border-white/[0.08] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50 transition-all" placeholder="Mínimo 8 caracteres" />
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <button type="button" onClick={handleCloseModal} className="px-4 py-2.5 border border-white/[0.08] text-white/70 rounded-xl text-sm font-medium hover:bg-white/[0.02] transition-colors">
                  Cancelar
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setSelectedGestor(null)}>
          <div className="bg-[#171717] rounded-2xl shadow-2xl w-full max-w-lg mx-4 animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              ...
              <button onClick={() => setSelectedGestor(null)} className="p-2 rounded-lg hover:bg-white/[0.04] text-white/30 transition-colors"><X className="h-5 w-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" defaultValue={selectedGestor.fullName || ''} id="edit-gestor-name" className="w-full px-4 py-2.5 border border-white/[0.08] rounded-xl text-sm capitalize focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-white/40 uppercase mb-1">Correo electrónico</label>
                  <input type="email" defaultValue={selectedGestor.email} id="edit-gestor-email" className="w-full px-4 py-2.5 border border-white/[0.08] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white/[0.02]/50" />
                </div>
              </div>
            </div>
            <div className="px-6 py-4 border-t flex justify-between">
              <button onClick={() => setSelectedGestor(null)} className="px-4 py-2.5 border border-white/[0.08] text-white/70 rounded-xl text-sm font-medium hover:bg-white/[0.02] transition-colors">Cerrar</button>
              <button
                onClick={async () => {
