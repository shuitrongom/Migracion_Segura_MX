...
const ESTATUS_BADGE: Record<string, string> = {
  ...
  completada: 'bg-white/[0.04] text-white/60 border-white/[0.08]',
...
export default function CitasPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-neutral-900 via-stone-800 to-amber-600 p-8 text-white shadow-xl">
      ...
      {showForm && (
        <div className="bg-[#171717] rounded-2xl border shadow-sm p-6 hover:shadow-md transition-shadow duration-300">
          <div className="flex items-center justify-between mb-5">
            ...
            <button type="button" onClick={() => setShowForm(false)} className="p-2 rounded-lg hover:bg-white/[0.04] transition-colors"><X className="h-4 w-4 text-white/40" /></button>
          </div>
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-white/60 mb-1.5">Extranjero *</label>
                <select value={form.clienteId} onChange={e => setForm(prev => ({ ...prev, clienteId: e.target.value }))} className="w-full px-4 py-2.5 border border-white/[0.08] bg-white/[0.02]/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500 capitalize transition-all">
              ...
              <div>
                ...
                <select value={form.gestorId} onChange={e => setForm(prev => ({ ...prev, gestorId: e.target.value }))} className="w-full px-4 py-2.5 border border-white/[0.08] bg-white/[0.02]/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500 capitalize transition-all">
              ...
              <div>
                ...
                <select value={form.tipo} onChange={e => setForm(prev => ({ ...prev, tipo: e.target.value, hora: '' }))} className="w-full px-4 py-2.5 border border-white/[0.08] bg-white/[0.02]/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select value={form.modalidad} onChange={e => setForm(prev => ({ ...prev, modalidad: e.target.value }))} className="w-full px-4 py-2.5 border border-white/[0.08] bg-white/[0.02]/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500 transition-all">
            ...
            <div>
              ...
              <input type="text" value={form.notas} onChange={e => setForm(prev => ({ ...prev, notas: e.target.value }))} className="w-full px-4 py-2.5 border border-white/[0.08] bg-white/[0.02]/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500 transition-all" placeholder="Observaciones..." />
            </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium border ${ESTATUS_BADGE[cita.estatus] || 'bg-white/[0.02] text-white/60 border-white/[0.08]'}`}>{cita.estatus}</span>
                      </div>
