...
export default function NuevoTramitePage() {
  ...
  const inputClass = (field: string) => `w-full px-3 py-2.5 border rounded-lg text-sm capitalize focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 ${hasError(field) ? 'border-red-500/60 bg-red-900/10' : 'border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 hover:border-slate-500'}`;
  const inputClassUpper = (field: string) => `w-full px-3 py-2.5 border rounded-lg text-sm uppercase focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 ${hasError(field) ? 'border-red-500/60 bg-red-900/10' : 'border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 hover:border-slate-500'}`;
  const inputClassEmail = (field: string) => `w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 ${hasError(field) ? 'border-red-500/60 bg-red-900/10' : 'border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 hover:border-slate-500'}`;
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <Link href="/tramites" className="p-2 rounded-lg hover:bg-[#112240] text-cyan-400 border border-cyan-900/50 transition-all hover:border-cyan-400/50 hover:shadow-lg hover:shadow-cyan-900/20"><ArrowLeft className="h-5 w-5" /></Link>
        <h1 className="text-2xl font-bold futuristic-gradient-text">Iniciar Trámite Migratorio</h1>
      </div>

      {/* Stepper futurista */}
      <div className="futuristic-card p-5 mb-6 relative overflow-hidden">
        <div className="absolute inset-0 futuristic-shimmer pointer-events-none" />
        <div className="flex items-center justify-between overflow-x-auto relative z-10">
          {STEPS.map((label, i) => (
            <div key={label} className="flex items-center gap-2 flex-shrink-0">
              <div className={`flex items-center justify-center h-9 w-9 rounded-full text-sm font-semibold transition-all duration-500 ${i < step ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30 futuristic-glow' : i === step ? 'bg-gradient-to-br from-cyan-400 to-blue-600 text-white shadow-lg shadow-cyan-500/30 ring-4 ring-cyan-500/20 futuristic-glow' : 'bg-[#112240] text-slate-500 border border-slate-700'}`}>
                {i < step ? <Check className="h-4 w-4" /> : i + 1}
              </div>
              <span className={`text-sm hidden lg:inline transition-colors duration-300 ${i === step ? 'font-semibold text-cyan-300' : i < step ? 'font-medium text-emerald-400' : 'text-slate-500'}`}>{label}</span>
              {i < STEPS.length - 1 && <div className={`hidden lg:block w-8 h-0.5 mx-2 rounded transition-all duration-500 ${i < step ? 'bg-emerald-400 shadow-sm shadow-emerald-400/50' : 'bg-slate-700'}`} />}
            </div>
          ))}
        </div>
      </div>

      <div className="futuristic-card p-6 relative overflow-hidden">
        ...
        {step === 0 && (
          <div className="relative z-10">
            <h3 className="text-xl font-bold text-cyan-300 mb-2">Selecciona el trámite</h3>
            <p className="text-sm text-slate-400 mb-6">Haz clic en el tipo de trámite para comenzar</p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {TRAMITES_INM.map((t, idx) => (
                <button key={t.tipo} type="button" onClick={() => { setSelectedTramite(t); setStep(1); }} className={`group relative text-left p-6 rounded-2xl border transition-all duration-500 overflow-hidden bg-[#0d1b2a]/80 border-slate-700/50 hover:border-cyan-400/60 hover:shadow-xl hover:shadow-cyan-900/20 hover:scale-[1.03] animate-fade-in-up stagger-${idx + 1}`} style={{ opacity: 0 }}>
                  {/* Número decorativo */}
                  <div className="absolute top-3 right-3 flex items-center justify-center h-8 w-8 rounded-full text-xs font-bold transition-all duration-500 bg-slate-800 text-slate-500 border border-slate-700 group-hover:bg-cyan-500/20 group-hover:text-cyan-300 group-hover:border-cyan-500/50 group-hover:shadow-lg group-hover:shadow-cyan-500/20">{idx + 1}</div>
                  {/* Barra lateral decorativa con glow */}
                  <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl transition-all duration-500 bg-slate-800 group-hover:bg-gradient-to-b group-hover:from-cyan-400 group-hover:to-blue-600 group-hover:shadow-[0_0_8px_rgba(0,212,255,0.5)]" />
                  <p className="text-base font-bold leading-tight pr-10 text-slate-200 group-hover:text-cyan-200 transition-colors">{t.nombre}</p>
                  <p className="text-xs text-slate-400 mt-2 leading-relaxed group-hover:text-slate-300 transition-colors">{t.descripcion}</p>
                  <div className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-cyan-400 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-[-8px] group-hover:translate-x-0">Comenzar →</div>
                </button>
              ))}
            </div>
          </div>
        ...
        {step === 1 && selectedTramite?.tipo !== 'constancia_empleador' && (
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Propósito del viaje</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Propósito de viaje *</label><select value={extranjero.propositoViaje} onChange={e => updateExtranjero('propositoViaje', e.target.value)} className={inputClass('propositoViaje')}><option value="">Selecciona</option>{PROPOSITOS_VIAJE.map(p => <option key={p} value={p}>{p}</option>)}</select><ErrorMsg field="propositoViaje" /></div>
              </div>
            </div>
            )}
            ...
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Tipo de trámite</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">¿Qué deseas hacer? *</label><select value={extranjero.propositoViaje} onChange={e => { updateExtranjero('propositoViaje', e.target.value); if (e.target.value !== 'Obtener permiso para trabajar') updateExtranjero('especificaTramite', ''); }} className={inputClass('propositoViaje')}><option value="">Selecciona</option><option value="Obtener permiso para trabajar">Obtener permiso para trabajar</option><option value="Obtener permiso de salida y regreso">Obtener permiso de salida y regreso</option></select><ErrorMsg field="propositoViaje" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Especifica *</label><select value={extranjero.especificaTramite} onChange={e => updateExtranjero('especificaTramite', e.target.value)} disabled={extranjero.propositoViaje !== 'Obtener permiso para trabajar'} className={`w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500 ${extranjero.propositoViaje !== 'Obtener permiso para trabajar' ? 'bg-slate-800 text-slate-500 cursor-not-allowed' : ''}`}><option value="">Selecciona</option><option value="Con empleador">Obtener permiso para trabajar con empleador</option><option value="Independiente/autoempleo">Obtener permiso para trabajar (actividades independientes/autoempleo)</option></select></div>
              </div>
            </div>
            )}
            ...
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Datos del extranjero (conforme a pasaporte o documento de identidad)</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mb-4">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Clave Única de Registro de Población (CURP)</label><input type="text" value={extranjero.curpExtranjero} onChange={e => updateExtranjero('curpExtranjero', e.target.value.toUpperCase())} className={inputClassUpper('curpExtranjero')} maxLength={18} />{extranjero.curpExtranjero && validateCurp(extranjero.curpExtranjero) && <p className="text-[11px] text-red-500 mt-1">{validateCurp(extranjero.curpExtranjero)}</p>}</div>
              </div>
              )}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Nombre(s) *</label><input type="text" value={extranjero.nombre} onChange={e => updateExtranjero('nombre', e.target.value)} className={inputClass('nombre')} /><ErrorMsg field="nombre" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Apellido(s) *</label><input type="text" value={extranjero.apellidos} onChange={e => updateExtranjero('apellidos', e.target.value)} className={inputClass('apellidos')} /><ErrorMsg field="apellidos" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Sexo *</label><select value={extranjero.sexo} onChange={e => updateExtranjero('sexo', e.target.value)} className={inputClass('sexo')}><option value="">Selecciona</option>{SEXOS.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select><ErrorMsg field="sexo" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Fecha de nacimiento *</label><DatePicker value={extranjero.fechaNacimiento} onChange={v => updateExtranjero('fechaNacimiento', v)} yearRange={[1940, 2010]} /><ErrorMsg field="fechaNacimiento" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Nacionalidad actual *</label><select value={extranjero.nacionalidad} onChange={e => updateExtranjero('nacionalidad', e.target.value)} className={inputClass('nacionalidad')}><option value="">Selecciona</option>{NACIONALIDADES.map(n => <option key={n} value={n}>{n}</option>)}</select><ErrorMsg field="nacionalidad" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Estado civil actual</label><select value={extranjero.estadoCivil} onChange={e => updateExtranjero('estadoCivil', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{ESTADOS_CIVILES.map(ec => <option key={ec.value} value={ec.value}>{ec.label}</option>)}</select></div>
              </div>
            </div>

            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Lugar de nacimiento</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">País de nacimiento *</label><select value={extranjero.paisNacimiento} onChange={e => updateExtranjero('paisNacimiento', e.target.value)} className={inputClass('paisNacimiento')}><option value="">Selecciona</option>{PAISES.map(p => <option key={p} value={p}>{p}</option>)}</select><ErrorMsg field="paisNacimiento" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Estado, provincia o departamento *</label><input type="text" value={extranjero.estadoProvinciaNacimiento} onChange={e => updateExtranjero('estadoProvinciaNacimiento', e.target.value)} className={inputClass('estadoProvinciaNacimiento')} /><ErrorMsg field="estadoProvinciaNacimiento" /></div>
              </div>
            </div>

            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Pasaporte o documento con el que se identifica el extranjero</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Documento de identificación *</label><select value={extranjero.documentoIdentificacion} onChange={e => updateExtranjero('documentoIdentificacion', e.target.value)} className={inputClass('documentoIdentificacion')}><option value="">Selecciona</option>{DOCUMENTOS_IDENTIFICACION.map(d => <option key={d} value={d}>{d}</option>)}</select><ErrorMsg field="documentoIdentificacion" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número de documento *</label><input type="text" value={extranjero.numeroDocumento} onChange={e => updateExtranjero('numeroDocumento', e.target.value)} className={inputClass('numeroDocumento')} /><ErrorMsg field="numeroDocumento" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">País de expedición *</label><select value={extranjero.paisExpedicion} onChange={e => updateExtranjero('paisExpedicion', e.target.value)} className={inputClass('paisExpedicion')}><option value="">Selecciona</option>{PAISES.map(p => <option key={p} value={p}>{p}</option>)}</select><ErrorMsg field="paisExpedicion" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Fecha de expedición</label><DatePicker value={extranjero.fechaExpedicion} onChange={v => updateExtranjero('fechaExpedicion', v)} yearRange={[2000, 2026]} /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Fecha de vencimiento</label><DatePicker value={extranjero.fechaVencimiento} onChange={v => updateExtranjero('fechaVencimiento', v)} yearRange={[2024, 2040]} /></div>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Domicilio del extranjero en México</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Código postal *</label><input type="text" value={extranjero.domCodigoPostal} onChange={e => updateExtranjero('domCodigoPostal', e.target.value)} className={inputClass('domCodigoPostal')} /><ErrorMsg field="domCodigoPostal" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Estado *</label><select value={extranjero.domEstado} onChange={e => { updateExtranjero('domEstado', e.target.value); updateExtranjero('domMunicipio', ''); }} className={inputClass('domEstado')}><option value="">Selecciona</option>{ESTADOS_MEXICO.map(est => <option key={est} value={est}>{est}</option>)}</select><ErrorMsg field="domEstado" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Municipio o Alcaldía *</label><select value={extranjero.domMunicipio} onChange={e => updateExtranjero('domMunicipio', e.target.value)} className={inputClass('domMunicipio')}><option value="">Selecciona</option>{(MUNICIPIOS_POR_ESTADO[extranjero.domEstado] || []).map(m => <option key={m} value={m}>{m}</option>)}</select><ErrorMsg field="domMunicipio" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Colonia *</label><input type="text" value={extranjero.domColonia} onChange={e => updateExtranjero('domColonia', e.target.value)} className={inputClass('domColonia')} /><ErrorMsg field="domColonia" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Calle *</label><input type="text" value={extranjero.domCalle} onChange={e => updateExtranjero('domCalle', e.target.value)} className={inputClass('domCalle')} /><ErrorMsg field="domCalle" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número exterior *</label><input type="text" value={extranjero.domNumeroExterior} onChange={e => updateExtranjero('domNumeroExterior', e.target.value)} className={inputClass('domNumeroExterior')} /><ErrorMsg field="domNumeroExterior" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número interior</label><input type="text" value={extranjero.domNumeroInterior} onChange={e => updateExtranjero('domNumeroInterior', e.target.value)} className="w-full px-3 py-2.5 border border-gray-300 bg-gray-50/50 rounded-lg text-sm capitalize focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 shadow-sm hover:shadow transition-shadow" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Lada</label><input type="text" value={extranjero.domLada} onChange={e => updateExtranjero('domLada', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Teléfono fijo</label><input type="text" value={extranjero.domTelefonoFijo} onChange={e => updateExtranjero('domTelefonoFijo', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
              </div>
            </div>
            )}
            ...
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Datos del empleador</h3>
              <div className="bg-cyan-900/20 border border-cyan-500/30 rounded-lg p-3 mb-4">
                <p className="text-xs text-cyan-300 text-center">Si presenta oferta de empleo, proporcione los datos del empleador.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Tipo de persona *</label><select value={extranjero.empleadorTipoPersona} onChange={e => updateExtranjero('empleadorTipoPersona', e.target.value)} className={inputClass('empleadorTipoPersona')}><option value="">Selecciona</option><option value="Física">Física</option><option value="Moral">Moral</option></select><ErrorMsg field="empleadorTipoPersona" /></div>
                {extranjero.empleadorTipoPersona && (
                <>
                  <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Registro Federal de Contribuyentes (RFC) *</label><input type="text" value={extranjero.empleadorRfc} onChange={e => updateExtranjero('empleadorRfc', e.target.value.toUpperCase())} className={inputClassUpper('empleadorRfc')} maxLength={extranjero.empleadorTipoPersona === 'Moral' ? 12 : 13} /><ErrorMsg field="empleadorRfc" /></div>
                  <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número expediente *</label><input type="text" value={extranjero.empleadorNumeroExpediente} onChange={e => updateExtranjero('empleadorNumeroExpediente', e.target.value)} className={inputClass('empleadorNumeroExpediente')} /><ErrorMsg field="empleadorNumeroExpediente" /></div>
                </>
                )}
              </div>
            </div>
            ...
            {selectedTramite?.tipo === 'visa' && (
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Información adicional del extranjero</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Actividad principal en tu país de residencia *</label><select value={extranjero.actividadPrincipal} onChange={e => updateExtranjero('actividadPrincipal', e.target.value)} className={inputClass('actividadPrincipal')}><option value="">Selecciona</option>{ACTIVIDADES_PRINCIPALES.map(a => <option key={a} value={a}>{a}</option>)}</select><ErrorMsg field="actividadPrincipal" /></div>
                {extranjero.actividadPrincipal === 'Trabajar' && (
                  <>
                    <div className="md:col-span-2"><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Sector o rama de actividad de la empresa o trabajo *</label><select value={extranjero.sectorTrabajo} onChange={e => updateExtranjero('sectorTrabajo', e.target.value)} className={inputClass('sectorTrabajo')}><option value="">Selecciona</option>{SECTORES_ACTIVIDAD.map(s => <option key={s} value={s}>{s}</option>)}</select><ErrorMsg field="sectorTrabajo" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Situación en el trabajo *</label><select value={extranjero.situacionTrabajo} onChange={e => updateExtranjero('situacionTrabajo', e.target.value)} className={inputClass('situacionTrabajo')}><option value="">Selecciona</option>{SITUACIONES_TRABAJO.map(s => <option key={s} value={s}>{s}</option>)}</select><ErrorMsg field="situacionTrabajo" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Ocupación en el trabajo *</label><select value={extranjero.ocupacionTrabajo} onChange={e => updateExtranjero('ocupacionTrabajo', e.target.value)} className={inputClass('ocupacionTrabajo')}><option value="">Selecciona</option>{OCUPACIONES_TRABAJO.map(o => <option key={o} value={o}>{o}</option>)}</select><ErrorMsg field="ocupacionTrabajo" /></div>
                  </>
                )}
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">¿Has sido expulsado de México? *</label><select value={extranjero.expulsadoMexico} onChange={e => updateExtranjero('expulsadoMexico', e.target.value)} className={inputClass('expulsadoMexico')}><option value="">Selecciona</option>{SI_NO.map(o => <option key={o} value={o}>{o}</option>)}</select><ErrorMsg field="expulsadoMexico" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">¿Tienes antecedentes penales? *</label><select value={extranjero.antecedentesPenales} onChange={e => updateExtranjero('antecedentesPenales', e.target.value)} className={inputClass('antecedentesPenales')}><option value="">Selecciona</option>{SI_NO.map(o => <option key={o} value={o}>{o}</option>)}</select><ErrorMsg field="antecedentesPenales" /></div>
              </div>
            </div>
            )}
            ...
            {selectedTramite?.tipo === 'visa' && (
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Señala las visas con las que cuenta el extranjero</h3>
              <div className="bg-cyan-900/20 border border-cyan-500/30 rounded-lg p-3 mb-4">
                <p className="text-xs text-cyan-300 text-center">Si deseas agregar visas será necesario que lo efectúes con el botón &apos;Agregar visa&apos;, de lo contrario los datos de esta sección no serán guardados.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl items-end">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">País</label><select value={visaTemp.pais} onChange={e => setVisaTemp(prev => ({ ...prev, pais: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{PAISES.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número</label><input type="text" value={visaTemp.numero} onChange={e => setVisaTemp(prev => ({ ...prev, numero: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                <div className="flex gap-2 items-end">
                  <div className="flex-1"><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Fecha de vencimiento</label><DatePicker value={visaTemp.vencimiento} onChange={v => setVisaTemp(prev => ({ ...prev, vencimiento: v }))} yearRange={[2024, 2040]} /></div>
            ...
            {selectedTramite?.tipo === 'visa' && (
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Datos de la institución, organismo o persona que solicita la autorización de la visa</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Tipo de persona *</label><select value={solicitante.tipoPersona} onChange={e => { updateSolicitante('tipoPersona', e.target.value); if (fieldErrors['tipoPersona']) setFieldErrors(prev => { const n = { ...prev }; delete n['tipoPersona']; return n; }); }} className={inputClass('tipoPersona')}><option value="">Selecciona</option>{TIPOS_PERSONA.map(t => <option key={t} value={t}>{t}</option>)}</select><ErrorMsg field="tipoPersona" /></div>
              </div>
              ...
              {solicitante.tipoPersona === 'Física' && (
                <div className="mt-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">CURP</label><input type="text" value={solicitante.curp} onChange={e => updateSolicitante('curp', e.target.value)} className={`${inputClassUpper('curp')} ${getCurpError(solicitante.curp) ? 'border-red-400 bg-red-50/30' : ''}`} maxLength={18} />{getCurpError(solicitante.curp) && <p className="text-[11px] text-red-500 mt-1">{getCurpError(solicitante.curp)}</p>}<CustomErrorMsg field="sol_curp" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">RFC</label><input type="text" value={solicitante.rfc} onChange={e => updateSolicitante('rfc', e.target.value)} className={`${inputClassUpper('rfc')} ${getRfcError(solicitante.rfc, false) ? 'border-red-400 bg-red-50/30' : ''}`} maxLength={13} />{getRfcError(solicitante.rfc, false) && <p className="text-[11px] text-red-500 mt-1">{getRfcError(solicitante.rfc, false)}</p>}<CustomErrorMsg field="sol_rfc" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Nombre(s) *</label><input type="text" value={solicitante.nombre} onChange={e => updateSolicitante('nombre', e.target.value)} className="w-full px-3 py-2.5 border border-gray-300 bg-gray-50/50 rounded-lg text-sm capitalize focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 shadow-sm hover:shadow transition-shadow" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Apellido(s) *</label><input type="text" value={solicitante.apellidos} onChange={e => updateSolicitante('apellidos', e.target.value)} className="w-full px-3 py-2.5 border border-gray-300 bg-gray-50/50 rounded-lg text-sm capitalize focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 shadow-sm hover:shadow transition-shadow" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Nacionalidad actual *</label><select value={solicitante.nacionalidad} onChange={e => updateSolicitante('nacionalidad', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{NACIONALIDADES.map(n => <option key={n} value={n}>{n}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Tipo de documento *</label><select value={solicitante.tipoDocumento} onChange={e => updateSolicitante('tipoDocumento', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{DOCUMENTOS_PERSONA_FISICA.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número de documento *</label><input type="text" value={solicitante.numeroDocumento} onChange={e => updateSolicitante('numeroDocumento', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div className="md:col-span-3"><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Vínculo o parentesco entre el solicitante y el extranjero *</label><select value={solicitante.vinculoParentesco} onChange={e => updateSolicitante('vinculoParentesco', e.target.value)} className={inputClass('sol_vinculoParentesco')}><option value="">Selecciona</option>{VINCULOS_PARENTESCO.map(v => <option key={v} value={v}>{v}</option>)}</select><ErrorMsg field="sol_vinculoParentesco" /></div>
                  </div>

                  <h4 className="text-sm font-semibold text-gray-800 border-b pb-1 pt-2">Domicilio de la persona física</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Código postal *</label><input type="text" value={solicitante.codigoPostal} onChange={e => updateSolicitante('codigoPostal', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Estado *</label><select value={solicitante.estado} onChange={e => { updateSolicitante('estado', e.target.value); updateSolicitante('municipio', ''); }} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{ESTADOS_MEXICO.map(est => <option key={est} value={est}>{est}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Municipio o Alcaldía *</label><select value={solicitante.municipio} onChange={e => updateSolicitante('municipio', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{(MUNICIPIOS_POR_ESTADO[solicitante.estado] || []).map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Colonia *</label><input type="text" value={solicitante.colonia} onChange={e => updateSolicitante('colonia', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Calle *</label><input type="text" value={solicitante.calle} onChange={e => updateSolicitante('calle', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número exterior *</label><input type="text" value={solicitante.numeroExterior} onChange={e => updateSolicitante('numeroExterior', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número interior</label><input type="text" value={solicitante.numeroInterior} onChange={e => updateSolicitante('numeroInterior', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Lada</label><input type="text" value={solicitante.lada} onChange={e => updateSolicitante('lada', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
              ...
              {solicitante.tipoPersona === 'Moral' && (
                <div className="mt-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    ...
                    <div className="md:col-span-2"><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Nombre o razón social *</label><input type="text" value={solicitante.moralRazonSocial} onChange={e => updateSolicitante('moralRazonSocial', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div className="md:col-span-3"><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Sector o rama de actividad</label><select value={solicitante.moralSector} onChange={e => updateSolicitante('moralSector', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{SECTORES_ACTIVIDAD.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                    <div className="md:col-span-3"><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Objeto de la empresa o giro comercial</label><textarea value={solicitante.moralGiroComercial} onChange={e => updateSolicitante('moralGiroComercial', e.target.value)} rows={3} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500 resize-none" /></div>
                  </div>

                  <h4 className="text-sm font-semibold text-gray-800 border-b pb-1 pt-2">Domicilio de la persona moral</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Código postal *</label><input type="text" value={solicitante.moralCodigoPostal} onChange={e => updateSolicitante('moralCodigoPostal', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Estado *</label><select value={solicitante.moralEstado} onChange={e => { updateSolicitante('moralEstado', e.target.value); updateSolicitante('moralMunicipio', ''); }} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{ESTADOS_MEXICO.map(est => <option key={est} value={est}>{est}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Municipio o Alcaldía *</label><select value={solicitante.moralMunicipio} onChange={e => updateSolicitante('moralMunicipio', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{(MUNICIPIOS_POR_ESTADO[solicitante.moralEstado] || []).map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Colonia *</label><input type="text" value={solicitante.moralColonia} onChange={e => updateSolicitante('moralColonia', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Calle *</label><input type="text" value={solicitante.moralCalle} onChange={e => updateSolicitante('moralCalle', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número exterior *</label><input type="text" value={solicitante.moralNumeroExterior} onChange={e => updateSolicitante('moralNumeroExterior', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número interior</label><input type="text" value={solicitante.moralNumeroInterior} onChange={e => updateSolicitante('moralNumeroInterior', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Lada</label><input type="text" value={solicitante.moralLada} onChange={e => updateSolicitante('moralLada', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Teléfono fijo</label><input type="text" value={solicitante.moralTelefonoFijo} onChange={e => updateSolicitante('moralTelefonoFijo', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                  </div>

                  <h4 className="text-sm font-semibold text-gray-800 border-b pb-1 pt-2">Datos del acta constitutiva</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número de acta constitutiva</label><input type="text" value={solicitante.moralNumeroActa} onChange={e => updateSolicitante('moralNumeroActa', e.target.value)} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                    <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Fecha de registro del acta</label><DatePicker value={solicitante.moralFechaActa} onChange={v => updateSolicitante('moralFechaActa', v)} yearRange={[1950, 2026]} /></div>
                  </div>
            ...
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Correo electrónico para notificar al promovente</h3>
              <div className="bg-cyan-900/20 border border-cyan-500/30 rounded-lg p-3 mb-4">
                <p className="text-xs text-cyan-300 text-center">Agrega la dirección de correo electrónico en donde se recibirán las notificaciones asociadas a tu trámite.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Correo electrónico *</label><input type="email" value={extranjero.solicitanteEmail} onChange={e => updateExtranjero('solicitanteEmail', e.target.value)} className={inputClassEmail('solicitanteEmail')} placeholder="nombre@correo.com" /><ErrorMsg field="solicitanteEmail" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Correo electrónico (confirmación) *</label><input type="email" value={extranjero.solicitanteEmailConfirmacion} onChange={e => updateExtranjero('solicitanteEmailConfirmacion', e.target.value)} className={inputClassEmail('solicitanteEmailConfirmacion')} placeholder="nombre@correo.com" /><ErrorMsg field="solicitanteEmailConfirmacion" /></div>
              </div>
            </div>

            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">En su caso, persona autorizada para tramitar, oír o recibir notificaciones</h3>
              <div className="bg-cyan-900/20 border border-cyan-500/30 rounded-lg p-3 mb-4">
                <p className="text-xs text-cyan-300 text-center">Si deseas agregar personas autorizadas es necesario que lo efectúes con el botón &apos;Agregar persona&apos;, de lo contrario los datos capturados en esta sección no serán guardados.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                ...
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Nombre(s) *</label><input type="text" value={personaTemp.nombre} onChange={e => setPersonaTemp(prev => ({ ...prev, nombre: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Apellido(s) *</label><input type="text" value={personaTemp.apellidos} onChange={e => setPersonaTemp(prev => ({ ...prev, apellidos: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Nacionalidad actual *</label><select value={personaTemp.nacionalidad} onChange={e => setPersonaTemp(prev => ({ ...prev, nacionalidad: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{NACIONALIDADES.map(n => <option key={n} value={n}>{n}</option>)}</select></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Tipo de documento de identificación</label><select value={personaTemp.tipoDocumento} onChange={e => setPersonaTemp(prev => ({ ...prev, tipoDocumento: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500"><option value="">Selecciona</option>{DOCUMENTOS_IDENTIFICACION_PERSONA.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número de documento</label><input type="text" value={personaTemp.numeroDocumento} onChange={e => setPersonaTemp(prev => ({ ...prev, numeroDocumento: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" /></div>
              </div>
            ...
            <div>
              <h3 className="text-xl font-bold text-cyan-300 mb-4 pb-3 border-b-2 border-cyan-500/30 futuristic-section-title">Comentarios</h3>
        ...
        {step === 2 && selectedTramite && (
          <div>
            ...
            <div className="mt-6 pt-6 border-t">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div>
                  <label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número de Pieza *</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Clave *</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">PDF Solicitud generada por el INM</label>
        ...
        {isAdmin && step === 4 && (
          <div>
            ...
            <div className="max-w-2xl space-y-5">
              {/* Concepto */}
              <div>
                <label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Concepto del pago *</label>
                <input type="text" value={pagoData.concepto || costo?.concepto || ''} onChange={e => setPagoData(prev => ({ ...prev, concepto: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" placeholder="Ej: Pago de derechos por visa" />
              </div>

              {/* Monto y Método */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Monto (MXN) *</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Método de pago *</label>
                  <select value={pagoData.metodoPago} onChange={e => setPagoData(prev => ({ ...prev, metodoPago: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500">
              ...
              {/* Referencia */}
              <div>
                <label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Número de referencia / folio</label>
                <input type="text" value={pagoData.referencia} onChange={e => setPagoData(prev => ({ ...prev, referencia: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-700 bg-[#0d1b2a]/80 text-cyan-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 shadow-sm transition-all duration-300 hover:border-slate-500" placeholder="Ej: REF-2025-001234" />
              </div>

              {/* Comprobante */}
              <div>
                <label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Comprobante de pago (PDF o imagen)</label>
      ...
      <div className="flex items-center justify-between mt-6">
        <button type="button" onClick={handleBack} disabled={step === 0} className="flex items-center gap-2 px-4 py-2.5 border border-slate-600 text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-800 hover:border-slate-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300"><ArrowLeft className="h-4 w-4" /> Anterior</button>
        {step < STEPS.length - 1 ? (
          <button type="button" onClick={handleNext} className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-lg text-sm font-semibold hover:from-cyan-400 hover:to-blue-500 shadow-lg shadow-cyan-500/25 transition-all duration-300 hover:shadow-cyan-500/40 hover:scale-[1.02]">Siguiente <ArrowRight className="h-4 w-4" /></button>
        ) : (
      ...
      {showCitaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4">
            ...
            <div className="space-y-4">
              <div><label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Fecha *</label><DatePicker value={citaData.fecha} onChange={v => setCitaData(prev => ({ ...prev, fecha: v }))} yearRange={[2025, 2027]} disablePast disableWeekends /></div>
              <div>
                <label className="block text-xs font-semibold text-cyan-200/70 mb-1 uppercase tracking-wide">Hora *</label>
