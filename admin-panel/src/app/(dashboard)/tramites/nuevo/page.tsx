...
export default function NuevoTramitePage() {
  ...
  const inputClassEmail = (field: string) => `w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 ${hasError(field) ? 'border-red-500/60 bg-red-900/10' : 'border-white/[0.08] bg-[#171717]/80 text-white hover:border-white/[0.15]'}`;
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <Link href="/tramites" className="p-2 rounded-lg hover:bg-white/[0.04] text-amber-400 border border-cyan-900/50 transition-all hover:border-amber-500/50 hover:shadow-lg hover:shadow-amber-900/20"><ArrowLeft className="h-5 w-5" /></Link>
        ...
        <div className="flex items-center justify-between overflow-x-auto relative z-10">
          {STEPS.map((label, i) => (
            <div key={label} className="flex items-center gap-2 flex-shrink-0">
              ...
              {i < STEPS.length - 1 && <div className={`hidden lg:block w-8 h-0.5 mx-2 rounded transition-all duration-500 ${i < step ? 'bg-emerald-400 shadow-sm shadow-emerald-400/50' : 'bg-white/[0.06]'}`} />}
            </div>
          ))}
        </div>
      </div>
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {TRAMITES_INM.map((t, idx) => (
                <button key={t.tipo} type="button" onClick={() => { setSelectedTramite(t); setStep(1); }} className={`group relative text-left p-6 rounded-2xl border transition-all duration-500 overflow-hidden bg-[#171717]/80 border-white/[0.08]/50 hover:border-amber-500/60 hover:shadow-xl hover:shadow-amber-900/20 hover:scale-[1.03] animate-fade-in-up stagger-${idx + 1}`} style={{ opacity: 0 }}>
                  {/* Número decorativo */}
                  <div className="absolute top-3 right-3 flex items-center justify-center h-8 w-8 rounded-full text-xs font-bold transition-all duration-500 bg-white/[0.04] text-white/30 border border-white/[0.08] group-hover:bg-cyan-500/20 group-hover:text-amber-400 group-hover:border-cyan-500/50 group-hover:shadow-lg group-hover:shadow-cyan-500/20">{idx + 1}</div>
                  {/* Barra lateral decorativa con glow */}
                  <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl transition-all duration-500 bg-white/[0.04] group-hover:bg-gradient-to-b group-hover:from-amber-400 group-hover:to-amber-600 group-hover:shadow-[0_0_8px_rgba(0,212,255,0.5)]" />
                  <p className="text-base font-bold leading-tight pr-10 text-white/80 group-hover:text-amber-300 transition-colors">{t.nombre}</p>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Propósito de viaje *</label><select value={extranjero.propositoViaje} onChange={e => updateExtranjero('propositoViaje', e.target.value)} className={inputClass('propositoViaje')}><option value="">Selecciona</option>{PROPOSITOS_VIAJE.map(p => <option key={p} value={p}>{p}</option>)}</select><ErrorMsg field="propositoViaje" /></div>
              </div>
            </div>
            )}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">¿Qué deseas hacer? *</label><select value={extranjero.propositoViaje} onChange={e => { updateExtranjero('propositoViaje', e.target.value); if (e.target.value !== 'Obtener permiso para trabajar') updateExtranjero('especificaTramite', ''); }} className={inputClass('propositoViaje')}><option value="">Selecciona</option><option value="Obtener permiso para trabajar">Obtener permiso para trabajar</option><option value="Obtener permiso de salida y regreso">Obtener permiso de salida y regreso</option></select><ErrorMsg field="propositoViaje" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Especifica *</label><select value={extranjero.especificaTramite} onChange={e => updateExtranjero('especificaTramite', e.target.value)} disabled={extranjero.propositoViaje !== 'Obtener permiso para trabajar'} className={`w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15] ${extranjero.propositoViaje !== 'Obtener permiso para trabajar' ? 'bg-white/[0.04] text-white/30 cursor-not-allowed' : ''}`}><option value="">Selecciona</option><option value="Con empleador">Obtener permiso para trabajar con empleador</option><option value="Independiente/autoempleo">Obtener permiso para trabajar (actividades independientes/autoempleo)</option></select></div>
              </div>
            </div>
            )}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mb-4">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Clave Única de Registro de Población (CURP)</label><input type="text" value={extranjero.curpExtranjero} onChange={e => updateExtranjero('curpExtranjero', e.target.value.toUpperCase())} className={inputClassUpper('curpExtranjero')} maxLength={18} />{extranjero.curpExtranjero && validateCurp(extranjero.curpExtranjero) && <p className="text-[11px] text-red-500 mt-1">{validateCurp(extranjero.curpExtranjero)}</p>}</div>
              </div>
              )}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Nombre(s) *</label><input type="text" value={extranjero.nombre} onChange={e => updateExtranjero('nombre', e.target.value)} className={inputClass('nombre')} /><ErrorMsg field="nombre" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Apellido(s) *</label><input type="text" value={extranjero.apellidos} onChange={e => updateExtranjero('apellidos', e.target.value)} className={inputClass('apellidos')} /><ErrorMsg field="apellidos" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Sexo *</label><select value={extranjero.sexo} onChange={e => updateExtranjero('sexo', e.target.value)} className={inputClass('sexo')}><option value="">Selecciona</option>{SEXOS.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select><ErrorMsg field="sexo" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Fecha de nacimiento *</label><DatePicker value={extranjero.fechaNacimiento} onChange={v => updateExtranjero('fechaNacimiento', v)} yearRange={[1940, 2010]} /><ErrorMsg field="fechaNacimiento" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Nacionalidad actual *</label><select value={extranjero.nacionalidad} onChange={e => updateExtranjero('nacionalidad', e.target.value)} className={inputClass('nacionalidad')}><option value="">Selecciona</option>{NACIONALIDADES.map(n => <option key={n} value={n}>{n}</option>)}</select><ErrorMsg field="nacionalidad" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Estado civil actual</label><select value={extranjero.estadoCivil} onChange={e => updateExtranjero('estadoCivil', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{ESTADOS_CIVILES.map(ec => <option key={ec.value} value={ec.value}>{ec.label}</option>)}</select></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">País de nacimiento *</label><select value={extranjero.paisNacimiento} onChange={e => updateExtranjero('paisNacimiento', e.target.value)} className={inputClass('paisNacimiento')}><option value="">Selecciona</option>{PAISES.map(p => <option key={p} value={p}>{p}</option>)}</select><ErrorMsg field="paisNacimiento" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Estado, provincia o departamento *</label><input type="text" value={extranjero.estadoProvinciaNacimiento} onChange={e => updateExtranjero('estadoProvinciaNacimiento', e.target.value)} className={inputClass('estadoProvinciaNacimiento')} /><ErrorMsg field="estadoProvinciaNacimiento" /></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Documento de identificación *</label><select value={extranjero.documentoIdentificacion} onChange={e => updateExtranjero('documentoIdentificacion', e.target.value)} className={inputClass('documentoIdentificacion')}><option value="">Selecciona</option>{DOCUMENTOS_IDENTIFICACION.map(d => <option key={d} value={d}>{d}</option>)}</select><ErrorMsg field="documentoIdentificacion" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número de documento *</label><input type="text" value={extranjero.numeroDocumento} onChange={e => updateExtranjero('numeroDocumento', e.target.value)} className={inputClass('numeroDocumento')} /><ErrorMsg field="numeroDocumento" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">País de expedición *</label><select value={extranjero.paisExpedicion} onChange={e => updateExtranjero('paisExpedicion', e.target.value)} className={inputClass('paisExpedicion')}><option value="">Selecciona</option>{PAISES.map(p => <option key={p} value={p}>{p}</option>)}</select><ErrorMsg field="paisExpedicion" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Fecha de expedición</label><DatePicker value={extranjero.fechaExpedicion} onChange={v => updateExtranjero('fechaExpedicion', v)} yearRange={[2000, 2026]} /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Fecha de vencimiento</label><DatePicker value={extranjero.fechaVencimiento} onChange={v => updateExtranjero('fechaVencimiento', v)} yearRange={[2024, 2040]} /></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Código postal *</label><input type="text" value={extranjero.domCodigoPostal} onChange={e => updateExtranjero('domCodigoPostal', e.target.value)} className={inputClass('domCodigoPostal')} /><ErrorMsg field="domCodigoPostal" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Estado *</label><select value={extranjero.domEstado} onChange={e => { updateExtranjero('domEstado', e.target.value); updateExtranjero('domMunicipio', ''); }} className={inputClass('domEstado')}><option value="">Selecciona</option>{ESTADOS_MEXICO.map(est => <option key={est} value={est}>{est}</option>)}</select><ErrorMsg field="domEstado" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Municipio o Alcaldía *</label><select value={extranjero.domMunicipio} onChange={e => updateExtranjero('domMunicipio', e.target.value)} className={inputClass('domMunicipio')}><option value="">Selecciona</option>{(MUNICIPIOS_POR_ESTADO[extranjero.domEstado] || []).map(m => <option key={m} value={m}>{m}</option>)}</select><ErrorMsg field="domMunicipio" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Colonia *</label><input type="text" value={extranjero.domColonia} onChange={e => updateExtranjero('domColonia', e.target.value)} className={inputClass('domColonia')} /><ErrorMsg field="domColonia" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Calle *</label><input type="text" value={extranjero.domCalle} onChange={e => updateExtranjero('domCalle', e.target.value)} className={inputClass('domCalle')} /><ErrorMsg field="domCalle" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número exterior *</label><input type="text" value={extranjero.domNumeroExterior} onChange={e => updateExtranjero('domNumeroExterior', e.target.value)} className={inputClass('domNumeroExterior')} /><ErrorMsg field="domNumeroExterior" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número interior</label><input type="text" value={extranjero.domNumeroInterior} onChange={e => updateExtranjero('domNumeroInterior', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.1] bg-white/[0.02]/50 rounded-lg text-sm capitalize focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 shadow-sm hover:shadow transition-shadow" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Lada</label><input type="text" value={extranjero.domLada} onChange={e => updateExtranjero('domLada', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Teléfono fijo</label><input type="text" value={extranjero.domTelefonoFijo} onChange={e => updateExtranjero('domTelefonoFijo', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
              </div>
            </div>
            )}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Actividad principal en tu país de residencia *</label><select value={extranjero.actividadPrincipal} onChange={e => updateExtranjero('actividadPrincipal', e.target.value)} className={inputClass('actividadPrincipal')}><option value="">Selecciona</option>{ACTIVIDADES_PRINCIPALES.map(a => <option key={a} value={a}>{a}</option>)}</select><ErrorMsg field="actividadPrincipal" /></div>
                {extranjero.actividadPrincipal === 'Trabajar' && (
                  <>
                    <div className="md:col-span-2"><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Sector o rama de actividad de la empresa o trabajo *</label><select value={extranjero.sectorTrabajo} onChange={e => updateExtranjero('sectorTrabajo', e.target.value)} className={inputClass('sectorTrabajo')}><option value="">Selecciona</option>{SECTORES_ACTIVIDAD.map(s => <option key={s} value={s}>{s}</option>)}</select><ErrorMsg field="sectorTrabajo" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Situación en el trabajo *</label><select value={extranjero.situacionTrabajo} onChange={e => updateExtranjero('situacionTrabajo', e.target.value)} className={inputClass('situacionTrabajo')}><option value="">Selecciona</option>{SITUACIONES_TRABAJO.map(s => <option key={s} value={s}>{s}</option>)}</select><ErrorMsg field="situacionTrabajo" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Ocupación en el trabajo *</label><select value={extranjero.ocupacionTrabajo} onChange={e => updateExtranjero('ocupacionTrabajo', e.target.value)} className={inputClass('ocupacionTrabajo')}><option value="">Selecciona</option>{OCUPACIONES_TRABAJO.map(o => <option key={o} value={o}>{o}</option>)}</select><ErrorMsg field="ocupacionTrabajo" /></div>
                  </>
                )}
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">¿Has sido expulsado de México? *</label><select value={extranjero.expulsadoMexico} onChange={e => updateExtranjero('expulsadoMexico', e.target.value)} className={inputClass('expulsadoMexico')}><option value="">Selecciona</option>{SI_NO.map(o => <option key={o} value={o}>{o}</option>)}</select><ErrorMsg field="expulsadoMexico" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">¿Tienes antecedentes penales? *</label><select value={extranjero.antecedentesPenales} onChange={e => updateExtranjero('antecedentesPenales', e.target.value)} className={inputClass('antecedentesPenales')}><option value="">Selecciona</option>{SI_NO.map(o => <option key={o} value={o}>{o}</option>)}</select><ErrorMsg field="antecedentesPenales" /></div>
              </div>
            </div>
            )}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl items-end">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">País</label><select value={visaTemp.pais} onChange={e => setVisaTemp(prev => ({ ...prev, pais: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{PAISES.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número</label><input type="text" value={visaTemp.numero} onChange={e => setVisaTemp(prev => ({ ...prev, numero: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                <div className="flex gap-2 items-end">
                  <div className="flex-1"><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Fecha de vencimiento</label><DatePicker value={visaTemp.vencimiento} onChange={v => setVisaTemp(prev => ({ ...prev, vencimiento: v }))} yearRange={[2024, 2040]} /></div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Tipo de persona *</label><select value={solicitante.tipoPersona} onChange={e => { updateSolicitante('tipoPersona', e.target.value); if (fieldErrors['tipoPersona']) setFieldErrors(prev => { const n = { ...prev }; delete n['tipoPersona']; return n; }); }} className={inputClass('tipoPersona')}><option value="">Selecciona</option>{TIPOS_PERSONA.map(t => <option key={t} value={t}>{t}</option>)}</select><ErrorMsg field="tipoPersona" /></div>
              </div>
              ...
              {solicitante.tipoPersona === 'Física' && (
                <div className="mt-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">CURP</label><input type="text" value={solicitante.curp} onChange={e => updateSolicitante('curp', e.target.value)} className={`${inputClassUpper('curp')} ${getCurpError(solicitante.curp) ? 'border-red-400 bg-red-50/30' : ''}`} maxLength={18} />{getCurpError(solicitante.curp) && <p className="text-[11px] text-red-500 mt-1">{getCurpError(solicitante.curp)}</p>}<CustomErrorMsg field="sol_curp" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">RFC</label><input type="text" value={solicitante.rfc} onChange={e => updateSolicitante('rfc', e.target.value)} className={`${inputClassUpper('rfc')} ${getRfcError(solicitante.rfc, false) ? 'border-red-400 bg-red-50/30' : ''}`} maxLength={13} />{getRfcError(solicitante.rfc, false) && <p className="text-[11px] text-red-500 mt-1">{getRfcError(solicitante.rfc, false)}</p>}<CustomErrorMsg field="sol_rfc" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Nombre(s) *</label><input type="text" value={solicitante.nombre} onChange={e => updateSolicitante('nombre', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.1] bg-white/[0.02]/50 rounded-lg text-sm capitalize focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 shadow-sm hover:shadow transition-shadow" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Apellido(s) *</label><input type="text" value={solicitante.apellidos} onChange={e => updateSolicitante('apellidos', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.1] bg-white/[0.02]/50 rounded-lg text-sm capitalize focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 shadow-sm hover:shadow transition-shadow" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Nacionalidad actual *</label><select value={solicitante.nacionalidad} onChange={e => updateSolicitante('nacionalidad', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{NACIONALIDADES.map(n => <option key={n} value={n}>{n}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Tipo de documento *</label><select value={solicitante.tipoDocumento} onChange={e => updateSolicitante('tipoDocumento', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{DOCUMENTOS_PERSONA_FISICA.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número de documento *</label><input type="text" value={solicitante.numeroDocumento} onChange={e => updateSolicitante('numeroDocumento', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div className="md:col-span-3"><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Vínculo o parentesco entre el solicitante y el extranjero *</label><select value={solicitante.vinculoParentesco} onChange={e => updateSolicitante('vinculoParentesco', e.target.value)} className={inputClass('sol_vinculoParentesco')}><option value="">Selecciona</option>{VINCULOS_PARENTESCO.map(v => <option key={v} value={v}>{v}</option>)}</select><ErrorMsg field="sol_vinculoParentesco" /></div>
                  </div>

                  <h4 className="text-sm font-semibold text-white/90 border-b pb-1 pt-2">Domicilio de la persona física</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Código postal *</label><input type="text" value={solicitante.codigoPostal} onChange={e => updateSolicitante('codigoPostal', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Estado *</label><select value={solicitante.estado} onChange={e => { updateSolicitante('estado', e.target.value); updateSolicitante('municipio', ''); }} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{ESTADOS_MEXICO.map(est => <option key={est} value={est}>{est}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Municipio o Alcaldía *</label><select value={solicitante.municipio} onChange={e => updateSolicitante('municipio', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{(MUNICIPIOS_POR_ESTADO[solicitante.estado] || []).map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Colonia *</label><input type="text" value={solicitante.colonia} onChange={e => updateSolicitante('colonia', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Calle *</label><input type="text" value={solicitante.calle} onChange={e => updateSolicitante('calle', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número exterior *</label><input type="text" value={solicitante.numeroExterior} onChange={e => updateSolicitante('numeroExterior', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número interior</label><input type="text" value={solicitante.numeroInterior} onChange={e => updateSolicitante('numeroInterior', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Lada</label><input type="text" value={solicitante.lada} onChange={e => updateSolicitante('lada', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
              ...
              {solicitante.tipoPersona === 'Moral' && (
                <div className="mt-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    ...
                    <div className="md:col-span-2"><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Nombre o razón social *</label><input type="text" value={solicitante.moralRazonSocial} onChange={e => updateSolicitante('moralRazonSocial', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div className="md:col-span-3"><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Sector o rama de actividad</label><select value={solicitante.moralSector} onChange={e => updateSolicitante('moralSector', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{SECTORES_ACTIVIDAD.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                    <div className="md:col-span-3"><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Objeto de la empresa o giro comercial</label><textarea value={solicitante.moralGiroComercial} onChange={e => updateSolicitante('moralGiroComercial', e.target.value)} rows={3} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15] resize-none" /></div>
                  </div>

                  <h4 className="text-sm font-semibold text-white/90 border-b pb-1 pt-2">Domicilio de la persona moral</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Código postal *</label><input type="text" value={solicitante.moralCodigoPostal} onChange={e => updateSolicitante('moralCodigoPostal', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Estado *</label><select value={solicitante.moralEstado} onChange={e => { updateSolicitante('moralEstado', e.target.value); updateSolicitante('moralMunicipio', ''); }} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{ESTADOS_MEXICO.map(est => <option key={est} value={est}>{est}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Municipio o Alcaldía *</label><select value={solicitante.moralMunicipio} onChange={e => updateSolicitante('moralMunicipio', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{(MUNICIPIOS_POR_ESTADO[solicitante.moralEstado] || []).map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Colonia *</label><input type="text" value={solicitante.moralColonia} onChange={e => updateSolicitante('moralColonia', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Calle *</label><input type="text" value={solicitante.moralCalle} onChange={e => updateSolicitante('moralCalle', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número exterior *</label><input type="text" value={solicitante.moralNumeroExterior} onChange={e => updateSolicitante('moralNumeroExterior', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número interior</label><input type="text" value={solicitante.moralNumeroInterior} onChange={e => updateSolicitante('moralNumeroInterior', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Lada</label><input type="text" value={solicitante.moralLada} onChange={e => updateSolicitante('moralLada', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Teléfono fijo</label><input type="text" value={solicitante.moralTelefonoFijo} onChange={e => updateSolicitante('moralTelefonoFijo', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                  </div>

                  <h4 className="text-sm font-semibold text-white/90 border-b pb-1 pt-2">Datos del acta constitutiva</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número de acta constitutiva</label><input type="text" value={solicitante.moralNumeroActa} onChange={e => updateSolicitante('moralNumeroActa', e.target.value)} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                    <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Fecha de registro del acta</label><DatePicker value={solicitante.moralFechaActa} onChange={v => updateSolicitante('moralFechaActa', v)} yearRange={[1950, 2026]} /></div>
                  </div>

                  {/* Confirmación RFC dado de alta en INM */}
                  <div className="mt-4 p-4 bg-white/[0.02] border border-white/[0.08] rounded-lg">
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input type="checkbox" checked={confirmRfcINM} onChange={e => setConfirmRfcINM(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-white/[0.1] text-amber-500 focus:ring-amber-500" />
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Clave Única de Registro de Población (CURP)</label><input type="text" value={personaTemp.curp} onChange={e => setPersonaTemp(prev => ({ ...prev, curp: e.target.value.toUpperCase() }))} className="w-full px-3 py-2 border border-white/[0.08] rounded-lg text-sm uppercase focus:outline-none focus:ring-2 focus:ring-amber-500" maxLength={18} />{personaTemp.curp && validateCurp(personaTemp.curp) && <p className="text-[11px] text-red-500 mt-1">{validateCurp(personaTemp.curp)}</p>}</div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Nombre(s) *</label><input type="text" value={personaTemp.nombre} onChange={e => setPersonaTemp(prev => ({ ...prev, nombre: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Apellido(s) *</label><input type="text" value={personaTemp.apellidos} onChange={e => setPersonaTemp(prev => ({ ...prev, apellidos: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Nacionalidad actual *</label><select value={personaTemp.nacionalidad} onChange={e => setPersonaTemp(prev => ({ ...prev, nacionalidad: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{NACIONALIDADES.map(n => <option key={n} value={n}>{n}</option>)}</select></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Tipo de documento de identificación</label><select value={personaTemp.tipoDocumento} onChange={e => setPersonaTemp(prev => ({ ...prev, tipoDocumento: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]"><option value="">Selecciona</option>{DOCUMENTOS_IDENTIFICACION_PERSONA.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número de documento</label><input type="text" value={personaTemp.numeroDocumento} onChange={e => setPersonaTemp(prev => ({ ...prev, numeroDocumento: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" /></div>
              </div>
            ...
            <div>
              ...
              <textarea value={extranjero.comentarios} onChange={e => updateExtranjero('comentarios', e.target.value)} rows={4} className="w-full max-w-4xl px-3 py-2 border border-white/[0.08] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 resize-none" />
        ...
        {step === 2 && selectedTramite && (
          <div>
                ...
                <div className="space-y-4">
                  ...
                  {extranjero.propositoViaje && (
                    <div>
                      ...
                      <button type="button" onClick={() => copyToClipboard(extranjero.propositoViaje)} className="w-full text-left p-1.5 rounded hover:bg-[#171717] border border-transparent hover:border-white/[0.08] transition-all group">
                      ...
                      <button type="button" onClick={() => copyToClipboard(extranjero.especificaTramite)} className="w-full text-left p-1.5 rounded hover:bg-[#171717] border border-transparent hover:border-white/[0.08] transition-all group mt-1">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {[
                          ...
                          <button key={item.label} type="button" onClick={() => copyToClipboard(item.value)} className="w-full text-left p-1.5 rounded hover:bg-[#171717] border border-transparent hover:border-white/[0.08] transition-all group">
                  ...
                  {visas.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-1">
                        {visas.map((v, i) => (
                          <button key={i} type="button" onClick={() => copyToClipboard(`${v.pais} - ${v.numero}`)} className="w-full text-left p-1.5 rounded hover:bg-[#171717] border border-transparent hover:border-white/[0.08] transition-all group">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {personasAutorizadas.map((p, i) => (
                          <button key={i} type="button" onClick={() => copyToClipboard(`${p.nombre} ${p.apellidos}`)} className="w-full text-left p-1.5 rounded hover:bg-[#171717] border border-transparent hover:border-white/[0.08] transition-all group">
            ...
            <div className="mt-6 pt-6 border-t">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl">
                <div>
                  <label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número de Pieza *</label>
                  <input type="text" value={numeroPieza} onChange={e => setNumeroPieza(e.target.value)} className="w-full px-3 py-2 border border-white/[0.08] rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-amber-500" placeholder="Ej: 0000011969016" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Clave *</label>
                  <input type="text" value={contrasenaINM} onChange={e => setContrasenaINM(e.target.value.toUpperCase())} className="w-full px-3 py-2 border border-white/[0.08] rounded-lg text-sm font-mono uppercase focus:outline-none focus:ring-2 focus:ring-amber-500" placeholder="Ej: QFCSA" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">PDF Solicitud generada por el INM</label>
        ...
        {step === 3 && (
          <div>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              ...
              <button type="button" onClick={() => setShowCitaModal(true)} className="flex items-center gap-2 px-4 py-2.5 border border-white/[0.08] text-white/70 rounded-lg text-sm font-medium hover:bg-white/[0.02] transition-colors">
        ...
        {isAdmin && step === 4 && (
          <div>
            ...
            <div className="max-w-2xl space-y-5">
              {/* Concepto */}
              <div>
                <label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Concepto del pago *</label>
                <input type="text" value={pagoData.concepto || costo?.concepto || ''} onChange={e => setPagoData(prev => ({ ...prev, concepto: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" placeholder="Ej: Pago de derechos por visa" />
              </div>

              {/* Monto y Método */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Monto (MXN) *</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30 text-sm">$</span>
                    <input type="number" value={pagoData.monto} onChange={e => setPagoData(prev => ({ ...prev, monto: e.target.value }))} className="w-full pl-7 pr-3 py-2.5 border border-white/[0.1] bg-white/[0.02]/50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 shadow-sm hover:shadow transition-shadow" placeholder="0.00" min="0" step="0.01" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Método de pago *</label>
                  <select value={pagoData.metodoPago} onChange={e => setPagoData(prev => ({ ...prev, metodoPago: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]">
              ...
              {/* Referencia */}
              <div>
                <label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Número de referencia / folio</label>
                <input type="text" value={pagoData.referencia} onChange={e => setPagoData(prev => ({ ...prev, referencia: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.08] bg-[#171717]/80 text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-amber-500 shadow-sm transition-all duration-300 hover:border-white/[0.15]" placeholder="Ej: REF-2025-001234" />
              </div>

              {/* Comprobante */}
              <div>
                <label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Comprobante de pago (PDF o imagen)</label>
                <div className="border-2 border-dashed border-white/[0.1] rounded-lg p-4 text-center hover:border-amber-500/50 transition-colors">
      ...
      <div className="flex items-center justify-between mt-6">
        <button type="button" onClick={handleBack} disabled={step === 0} className="flex items-center gap-2 px-4 py-2.5 border border-white/[0.1] text-white/60 rounded-lg text-sm font-medium hover:bg-white/[0.04] hover:border-white/[0.15] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300"><ArrowLeft className="h-4 w-4" /> Anterior</button>
        {step < STEPS.length - 1 ? (
      ...
      {showCitaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-[#171717] rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4">
            ...
            <div className="space-y-4">
              <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Fecha *</label><DatePicker value={citaData.fecha} onChange={v => setCitaData(prev => ({ ...prev, fecha: v }))} yearRange={[2025, 2027]} disablePast disableWeekends /></div>
              <div>
                <label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Hora *</label>
                <input type="time" value={citaData.horaInicio} onChange={e => setCitaData(prev => ({ ...prev, horaInicio: e.target.value }))} className="w-full px-3 py-2.5 border border-white/[0.1] bg-white/[0.02]/50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 shadow-sm" />
              </div>
              <div><label className="block text-xs font-semibold text-amber-300/70 mb-1 uppercase tracking-wide">Notas</label><textarea value={citaData.notas} onChange={e => setCitaData(prev => ({ ...prev, notas: e.target.value }))} rows={3} className="w-full px-3 py-2.5 border border-white/[0.1] bg-white/[0.02]/50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 shadow-sm resize-none" placeholder="Ej: Entrega de documentos originales en oficina" /></div>
            </div>
            <div className="flex justify-end gap-3 mt-6">
              <button type="button" onClick={() => setShowCitaModal(false)} className="px-4 py-2 border border-white/[0.08] text-white/70 rounded-lg text-sm font-medium hover:bg-white/[0.02]">Cancelar</button>
