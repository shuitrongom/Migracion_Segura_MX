...
export default function ExtranjerosScreen() {
  ...
  const loadClientes = async () => {
    try {
      ...
      if (res.ok) { const data = await res.json(); setClientes(data.data || []); }
    } catch {}
    setLoading(false);
  };
      ...
      <Animated.View style={{ flex: 1, opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
        <View style={styles.searchContainer}>
          <TextInput style={[styles.searchInput, { backgroundColor: inputBg, borderColor: inputBorder, color: colors.text }]} value={search} onChangeText={setSearch}
            placeholder="Buscar por nombre, email o nacionalidad..." placeholderTextColor={colors.textMuted} />
        </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={[styles.emptyText, { color: colors.textMuted }]}>No hay extranjeros registrados</Text>
            </View>
          }
          renderItem={({ item }) => (
            <TouchableOpacity style={[styles.card, { backgroundColor: cardBg, borderColor: cardBorder }]}>
              <View style={styles.cardHeader}>
                ...
                <View style={styles.cardInfo}>
                  <Text style={[styles.cardName, { color: colors.text }]}>{item.nombreCompleto || `${item.nombre || ''} ${item.apellidos || ''}`}</Text>
                  {item.nacionalidad && <Text style={[styles.cardNacionalidad, { color: colors.textMuted }]}>🌍 {item.nacionalidad}</Text>}
                </View>
              </View>
              <View style={styles.cardDetails}>
                <Text style={[styles.detailText, { color: colors.textSecondary }]}>📧 {item.email}</Text>
                {item.telefono && <Text style={[styles.detailText, { color: colors.textSecondary }]}>📱 {item.telefono}</Text>}
              </View>
            </TouchableOpacity>
          )}
        />
...
const styles = StyleSheet.create({
  ...
  cardNacionalidad: { fontSize: 13, marginTop: 2 },
